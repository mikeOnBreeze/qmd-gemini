...
/**
 * Detect what embedding model was used for the stored vectors.
 * Returns the model name or null if no vectors exist.
 */
export function detectEmbeddingModel(db: Database): string | null {
  const result = db.prepare(`
    SELECT model FROM content_vectors LIMIT 1
  `).get() as { model: string } | null;
  return result?.model || null;
}

/**
 * Check if the stored embeddings were created with Gemini.
 */
export function isGeminiEmbeddings(db: Database): boolean {
  const model = detectEmbeddingModel(db);
  return model?.startsWith("gemini") || false;
}

// Lazy-initialized Gemini embedder for query embedding
let _geminiEmbedder: GeminiEmbedder | null = null;

function getGeminiEmbedderForSearch(): GeminiEmbedder {
  if (!_geminiEmbedder) {
    _geminiEmbedder = createGeminiEmbedder();
  }
